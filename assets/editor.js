...
( function () {
	...
	window.addEventListener( 'message', function ( event ) {
		...
		if ( 'navigate' === event.data.type ) {
			...
			/*
			 * Своя страница: перезагружаем редактор целиком, а не подменяем
			 * адрес фрейма. Подмена оставляла бы панель с данными прошлой
			 * страницы — тот же `postId`, тот же список строк, — и «Перевести
			 * весь материал» ушло бы в предыдущую запись без единого признака,
			 * что что-то не так. Заодно предпросмотр получает свежий nonce и
			 * токен канала: без них он снова открылся бы как обычная страница
			 * сайта, без разметки редактора.
			 *
			 * Разбирать адрес в путь — забота сервера (см. pathFromUrl()):
			 * вторая реализация этого правила здесь неизбежно разошлась бы
			 * с первой.
			 */
			var next = new URL( window.location.href );

			next.searchParams.set( 'mlp_url', event.data.url );
