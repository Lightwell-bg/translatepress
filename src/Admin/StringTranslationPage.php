	...
	private function renderStringsTab( string $tab, array $secondary, array $filters ): void {
		...
		<p class="description">
			<?php if ( self::TAB_SEO === $tab ) : ?>
				<?php esc_html_e( 'Поля для поисковиков и превью в соцсетях: meta description, Open Graph, Twitter Card и текстовые поля JSON-LD. На самой странице они не видны.', 'wp-mlp' ); ?>
			<?php else : ?>
				<?php esc_html_e( 'Тексты сайта: записи, страницы, пункты меню, виджеты, подписи к картинкам и кнопки. Строки появляются здесь после того, как вы откроете страницу на дополнительном языке.', 'wp-mlp' ); ?>
			<?php endif; ?>
		</p>
