...
final class SourceRepository {
	...
	/**
	 * Знак подстановки в поисковой фразе: «любое количество любых символов».
	 *
	 * Тот же символ, что в поиске файлов Windows/Проводника и в macOS
	 * Spotlight, — не своё изобретение, а то, что уже знакомо любому,
	 * кто когда-либо искал файл по маске.
	 */
	private const WILDCARD = '*';

	/**
	 * Содержит ли строка искомую фразу. Чистая функция.
	 *
	 * Два режима, в зависимости от того, есть ли в фразе `*`:
	 *
	 * 1. **Без `*`** — фраза целиком, с границей слова там, где сама
	 *    фраза заканчивается буквой или цифрой (см. wordBoundaryPattern()).
	 *    «AI» находит «AI Act», но не «domain» — по умолчанию для обычного
	 *    ввода без спецсимволов.
	 * 2. **С `*`** — маска, привязанная к КРАЯМ строки целиком: `*` в
	 *    начале означает «строка кончается на …», в конце — «начинается
	 *    с …», с обеих сторон — «содержит …» без границы слова. Только
	 *    маска умеет то, что не может граница слова в принципе: отличить
	 *    `/blog/en/` в конце адреса от той же подстроки посередине более
	 *    длинного пути — граница слова видит оба случая как «слово»,
	 *    маска `*blog/en/` — только первый.
	 *
	 * @param string $haystack Текст, в котором ищем.
	 * @param string $search   Искомая фраза или маска с `*`.
	 */
	public static function matchesSearchPhrase( string $haystack, string $search ): bool {
		...
		$pattern = str_contains( $search, self::WILDCARD )
			? self::wildcardPattern( $search )
			: self::wordBoundaryPattern( $search );

		return 1 === preg_match( $pattern, $haystack );
	}

	/**
	 * Регулярное выражение для обычной фразы: граница нужна только с той
	 * стороны, где сама фраза заканчивается буквой или цифрой.
	 *
	 * `\p{L}`/`\p{N}` — буква или цифра ЛЮБОГО алфавита в Unicode, не
	 * только латиница: граница проверяется одинаково что для «AI», что
	 * для «плагин».
	 *
	 * Без «только с той стороны, где есть чем слипаться» поиск адреса
	 ...
	 *
	 * @param string $search Искомая фраза, без `*`.
	 */
	private static function wordBoundaryPattern( string $search ): string {
		$quoted = preg_quote( $search, '/' );
		$before = 1 === preg_match( '/^[\p{L}\p{N}]/u', $search ) ? '(?<![\p{L}\p{N}])' : '';
		$after  = 1 === preg_match( '/[\p{L}\p{N}]$/u', $search ) ? '(?![\p{L}\p{N}])' : '';

		return "/{$before}{$quoted}{$after}/iu";
	}

	/**
	 * Регулярное выражение для маски с `*`: обе стороны привязаны к краям
	 * строки, а `*` разворачивается в «сколько угодно любых символов».
	 *
	 * Привязка безусловная (в отличие от wordBoundaryPattern()) — маска не
	 * про слово, а про точное место в строке, и `s`-модификатор гарантирует,
	 * что `*` не споткнётся о случайный перенос строки внутри длинного
	 * текста.
	 *
	 * @param string $search Маска с `*`.
	 */
	private static function wildcardPattern( string $search ): string {
		$parts = array_map(
			static fn( string $part ): string => preg_quote( $part, '/' ),
			explode( self::WILDCARD, $search )
		);

		return '/^' . implode( '.*', $parts ) . '$/isu';
	}
